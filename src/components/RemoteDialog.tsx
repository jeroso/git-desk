...
export function RemoteDialog({ repo, onClose }: { repo: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-40">
      <div className="bg-white dark:bg-neutral-800 dark:text-neutral-100 rounded-lg shadow-xl w-[28rem] p-4 text-xs space-y-3">
        <div className="font-semibold text-sm">Remote &amp; Account</div>
        <div>
          <div className="text-gray-500 dark:text-neutral-400">Remote: {origin?.name ?? '(없음)'}</div>
          <div className="font-mono break-all">{origin?.url ?? '-'}</div>
        </div>
        <div>
          <div className="text-gray-500 dark:text-neutral-400 mb-1">계정 (~/.ssh/config):</div>
          {hosts.length === 0 && <div className="text-gray-400 dark:text-neutral-500">Host 항목이 없습니다</div>}
          {hosts.map((h) => (
        ...
        <div className="text-gray-400 dark:text-neutral-500">계정 선택 시 URL의 호스트 별칭이 자동 변경됩니다.</div>
        <div className="flex justify-end gap-2 pt-2">
          <button onClick={onClose} className="border dark:border-neutral-600 rounded px-3 py-1">
            취소
          </button>
          <button
            onClick={apply}
